...
| `sources` (playbook)                           | Swift files or folders to scan for Playbook previews. Replaces the files of the target the plugin is attached to, so list that target too. Use it to build one catalog from several local packages. Views used in previews from other modules must be `public`, and the target must depend on those modules. |
